...
import argparse, iris, os, datetime
...
def saveiris(args,cube,variable_name):
	...
	saver.write(cube,local_keys=['comment', 'associated_files', 'coordinates', 'missing_value','conversion_factor','cell_measures'])
...
def main(args):
	...
	cubes = map(reqd.convert,cubes)
	
	# Convert to pressure levels
	newdata = reqd.convert_pressure(iris.cube.CubeList(cubes))
